...
from conversion import transform_vedio as T
...
class A2DSentencesDataset(Dataset):
    ...
    def __init__(self, ann_file: Path, transforms, return_masks: bool,
                 num_frames: int, max_skip: int, subset):
        super(A2DSentencesDataset, self).__init__()
        dataset_path = '/13994058190/WYH/UNINEXT/datasets/a2d_sentences'
    ...
    def __getitem__(self, idx):
        ...
        while not instance_check:
            ...
            if self.subset == 'train':
                ...
                file_names = []
                for frame_ids in sample_indx:
                    file_name = f'{prefix}/{frame_ids:05d}.png'
                    file_names.append(file_name)
                # file_name = f'{prefix}/{frame_id:05d}.png'
                # file_names.append(file_name)
            ...
            mask_rles[0]['counts'] = mask_rles[0]['counts'].decode('utf-8')
            # there is only one valid frame
            target = {
                'file_names': file_names,  # [T,]
                'valid': torch.tensor(valid),  # [1,]
                'boxes': [boxes.squeeze(0).tolist() for i in range(len(file_names))],  # [1, 4], xyxy
                'masks': [mask_rles[0] for i in range(len(file_names))],  # [H, W]
                'area': [mask_areas[0] for i in range(len(file_names))],  # [1,]
                'caption': text_query,
...
# 创建该数据集，并遍历一边，把得到的数据保存到json文件中
def create_json(pattern='train'):
    # 读取数据集
    if pattern == 'train':
        ann_file = Path(
            '/13994058190/WYH/UNINEXT/datasets/a2d_sentences/a2d_sentences_single_frame_train_annotations.json')
    else:
        ann_file = Path(
            '/13994058190/WYH/UNINEXT/datasets/a2d_sentences/a2d_sentences_single_frame_test_annotations.json')
    dataset = A2DSentencesDataset(ann_file=ann_file,
                                  transforms=make_coco_transforms(image_set='train', max_size=640),
                                  return_masks=True, num_frames=20, max_skip=0, subset=pattern)
    # 遍历数据集
    ...
    for i in range(len(dataset)):
        ...
        # 按照coco数据集的格式保存
        inst_idx += 1
        video = {"height": target['size'][0], "width": target['size'][1], "length": target['length'],
                 "file_names": target['file_names'],
                 "id": inst_idx, "expressions": target['caption']}
        anno = {"video_id": inst_idx, "id": inst_idx, "iscrowd": 0, "category_id": 1,
                "bboxes": target['boxes'], 'segmentations': target['masks'],
                "areas": target['area']}
        new_data["videos"].append(video)
        new_data["annotations"].append(anno)
        print(i)
    output_json = f'/13994058190/WYH/UNINEXT/datasets/a2d_sentences/instances_{pattern}.json'
    json.dump(new_data, open(output_json, 'w'))


create_json(pattern='train')
create_json(pattern='val')
