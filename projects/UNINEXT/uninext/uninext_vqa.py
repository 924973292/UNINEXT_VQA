...
class UNINEXT_VQA(nn.Module):
    """
    Unified model for image-level VQA task
    """
    ...
    def __init__(self, cfg):
        ...
        self.tokenizer = AutoTokenizer.from_pretrained("/13994058190/WYH/UNINEXT/projects/UNINEXT/bert-base-uncased")
        ...
        # vs = 3129  # number of vqav2_label_size
        # hs = hidden_dim
    ...
    def forward(self, batched_inputs, do_postprocess=False):
        """
        Args:
        ...
        if self.training:
            ...
            output, loss_dict, language_cash, visual_cash = self.detr.coco_forward(images, targets, self.criterion,
                                                                                   train=True,
            ...
            output, loss_dict = self.detr.coco_inference(images, None, self.criterion, train=False,
                                                         language_dict_features=language_dict_features, task=task)
            ...
            results = self.inference(box_cls, box_pred, mask_pred, images.image_sizes, positive_map_label_to_token,
                                     num_classes, task=task, iou_pred=iou_pred)
            if do_postprocess:
                processed_results = []
    ...
    def inference(self, box_cls, box_pred, mask_pred, image_sizes, positive_map_label_to_token, num_classes,
                  score_thres=0.0, task=None, iou_pred=None):
        """
        Arguments:
        ...
        for i, (logits_per_image, box_pred_per_image, image_size, iou_per_image) in enumerate(zip(
            ...
            if self.ota:
                # NMS
                logits_per_image = convert_grounding_to_od_logits(logits_per_image.unsqueeze(0), num_classes,
        ...
        if isinstance(captions[0], str):
            ...
            if self.parallel_det:
                language_dict_features = self.text_encoder(tokenizer_input,
                                                           task=task)  # dict with keys: ['aggregate', 'embedded', 'masks', 'hidden']
            else:
                language_dict_features = self.text_encoder(
                    tokenizer_input)  # dict with keys: ['aggregate', 'embedded', 'masks', 'hidden']
