...
class DDETRSegmUniVIDDN(DDETRSegmUniDN):
    def __init__(self, detr, rel_coord=True, ota=False, new_mask_head=False, use_raft=False, mask_out_stride=4, \
        ...
        super().__init__(detr, rel_coord=rel_coord, ota=ota, new_mask_head=new_mask_head, use_raft=use_raft,
        ...
        self.use_deformable_reid_head = cfg.USE_DEFORMABLE_REID_HEAD
        # transform the language features and visual features to the same dimension
        self.language_embed = nn.Linear(768, 512)
        self.visual_embed = nn.Linear(256, 512)
    ...
    def get_template(self, img, mask, bbox):
        ...
        # resize
        im_crop_padded = F.interpolate(im_crop_padded, (self.template_sz, self.template_sz), mode='bilinear',
                                       align_corners=False)
        mask_crop_padded = \
        F.interpolate(mask_crop_padded, (self.template_sz, self.template_sz), mode='bilinear', align_corners=False)[
            0].bool()
    ...
    def get_template_4c(self, img, pad_mask, bbox, gt_mask=None):
        ...
        im_crop_padded = F.pad(im_crop, pad=(x1_pad, x2_pad, y1_pad, y2_pad), mode='constant', value=0)
        mask_crop_padded = F.pad(mask_crop, pad=(x1_pad, x2_pad, y1_pad, y2_pad), mode='constant', value=1).float()
        mask_crop_padded_gt = F.pad(mask_crop_gt, pad=(x1_pad, x2_pad, y1_pad, y2_pad), mode='constant',
                                    value=0).float()
        ...
        F.interpolate(mask_crop_padded, (self.template_sz, self.template_sz), mode='bilinear', align_corners=False)[
            0].bool()
        mask_crop_padded_gt = F.interpolate(mask_crop_padded_gt, (self.template_sz, self.template_sz), mode='bilinear',
                                            align_corners=False)
    ...
    def coco_forward_sot(self, samples, det_targets, ref_targets, criterion, train=False, task=None):
        ...
        if self.sot_feat_fusion:
            ...
            for n_l in range(self.detr.num_feature_levels):
                ...
                ref_mask_l = F.interpolate(masks_ref[n_l][None].float(), size=(self.ref_feat_sz, self.ref_feat_sz))[
                    0].bool()  # (bs, 8, 8)
        ...
        language_dict_features_key["hidden"], language_dict_features_key["masks"] = self.adjust_layer(
            ref_feats), ~ref_masks
        ...
        if self.dynamic_label_enc:
            lang_feat_pool = agg_lang_feat(language_dict_features_key["hidden"], language_dict_features_key["masks"],
                                           pool_type=self.cls_pool_type)  # (bs, 768)
        ...
        hs, memory, init_reference, inter_references, enc_outputs_class, enc_outputs_coord_unact, language_dict_features_key = \
            self.detr.transformer(srcs_key, masks_key, poses_key, query_embeds, mask_on=True,
                                  language_dict_features=language_dict_features_key, task=task,
                                  attn_masks=attn_mask)

        if task == "grounding" or task == "sot":
            lang_feat_pool_key = agg_lang_feat(language_dict_features_key["hidden"],
                                               language_dict_features_key["masks"]).unsqueeze(1)  # (bs, 1, 768)
    ...
    def coco_forward_vis(self, samples, det_targets, ref_targets, criterion, train=False, language_dict_features=None,
                         task=None):
        image_sizes = samples.image_sizes
        if not isinstance(samples, NestedTensor):
            samples = nested_tensor_from_tensor_list(samples, size_divisibility=32)
        ...
        if self.use_deformable_reid_head:
            ...
            hs_ref, _, _, inter_references_ref, _un1, _un2, language_dict_features_ref, src_info_ref = \
                self.detr.transformer(srcs_reference, masks_reference, poses_reference, [None, None], mask_on=True,
                                      language_dict_features=language_dict_features_ref, task=task,
                                      return_src_info=True)
            visual_key = self.visual_embed(torch.mean(hs[-1], dim=1))
            visual_ref = self.visual_embed(torch.mean(hs_ref[-1], dim=1))
            visual_cash = torch.cat([visual_key, visual_ref], dim=0)
            if dn_meta and dn_meta["single_padding"] > 0:
                padding_size = dn_meta["single_padding"] * dn_meta["dn_num"]
        ...
        if task == "grounding":
            lang_feat_pool_key = agg_lang_feat(language_dict_features_key["hidden"],
                                               language_dict_features_key["masks"]).unsqueeze(1)  # (bs, 1, 768)
            lang_feat_pool_ref = agg_lang_feat(language_dict_features_ref["hidden"],
                                               language_dict_features_ref["masks"]).unsqueeze(1)  # (bs, 1, 768)
        elif task == "detection":
            pass
        else:
            raise ValueError("task must be detection or grounding")
        ...
        language_cash = self.language_embed(torch.mean(language_dict_features_key["hidden"], dim=1))  # (bs, 512)
        ...
        if task == "grounding":
            ...
            ref_cls = self.detr.class_embed[enc_lay_num - 1](hs_ref[-1], language_dict_features_ref[
                "hidden"]).sigmoid()  # NOTE: self.detr.class_embed[-1] may cause conflict with two-stage !

        # Only compute Embedding Loss on the last layer of decoder
        contrast_items = select_pos_neg(inter_references_ref[-1], matched_ids, ref_targets, det_targets,
                                        self.reid_embed_head, hs[-1][:, padding_size:], hs_ref[-1], ref_cls,
                                        detach_reid=self.detach_reid,
                                        use_deformable_reid_head=self.use_deformable_reid_head,
                                        src_info_key=src_info_key, src_info_ref=src_info_ref)
        ...
        if no_valid_obj:
            ...
            for i in range(enc_lay_num - 1):
                loss_dict["loss_mask_%d" % i] = loss_mask
                loss_dict["loss_dice_%d" % i] = loss_mask
        return outputs, loss_dict, visual_cash, language_cash
