...
class DDETRSegmUniDN(nn.Module):
    ...
    def coco_forward(self, samples, gt_targets, criterion, train=False, language_dict_features=None, task=None):
        ...
        language_cash = self.language_embed(torch.mean(language_dict_features["hidden"],dim=1))
        visual_cash = self.visual_embed(torch.mean(hs[-1],dim=1))
        ...
        for lvl in range(enc_lay_num):
            ...
            #if false in has_mask_list，then all false
            if not all(has_mask_list):
                has_mask_list = [False for x in gt_targets]
        ...
        return outputs, loss_dict,language_cash,visual_cash
